...
class LLMConfig {
  ...
  static Future<LLMConfig?> load() async {
    try {
      ...
      final data = prefs.getString('vias_llm_config');
      if (data != null && data.isNotEmpty) {
        return LLMConfig.fromJson(jsonDecode(data) as Map<String, dynamic>);
      }
    } catch (_) {}
    return null;
  }
  ...
  Future<void> save() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('vias_llm_config', jsonEncode(toJson()));
    } catch (_) {}
  }
...
class Conversation {
  ...
  String exportAsMarkdown() {
    ...
    buf.writeln('# $title\n');
    buf.writeln('> Generated by Trae Agent\n');
    for (final msg in messages) {
      buf.writeln('---\n');
      buf.writeln(msg.toMarkdown());
      buf.writeln('');
    }
    return buf.toString();
  }
